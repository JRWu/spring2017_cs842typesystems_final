function typed_concatenate(x:string, y:string){
	return x + y
}

function typed_levnshtein (s:string , t:string) {
        if (s.length === 0) return t.length;
        if (t.length === 0) return s.length;

        return Math.min(
                typed_levnshtein(s.substr(1), t) + 1,
                typed_levnshtein(t.substr(1), s) + 1,
                typed_levnshtein(s.substr(1), t.substr(1)) + (s[0] !== t[0] ? 1 : 0)
        );
}
